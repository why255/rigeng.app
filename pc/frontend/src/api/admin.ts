...
export interface AlgorithmFileItem {
  id: string
  module_key: string
...
export function getAlgorithmModules() {
  return apiGet<AlgorithmModule[]>('/admin/algorithms/modules')
}

export function getAlgorithms(moduleKey?: string) {
  const params = moduleKey ? { module_key: moduleKey } : undefined
  return apiGet<AlgorithmFileItem[]>('/admin/algorithms', params)
}

export function uploadAlgorithm(moduleKey: string, filename: string, content: string) {
  ...
  return fetch('/api/v1/admin/algorithms/upload', {
    method: 'POST',
