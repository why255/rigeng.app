/**
 * M12-P2 文档查看与预览页
 * 对齐 m12-p2.html：面包屑、搜索、分类目录树、文档表格、右侧预览面板（携君库水印）
 */
import { useEffect, useState, useCallback } from 'react';
import { useNavigate, useSearchParams, Link } from 'react-router-dom';
import { PageContainer } from '@/components/layout/AppShell';
import { Card, Text, Tag, Button } from '@/components/primitives';
import { useToast } from '@/components/primitives/toast';
import * as knowledgeApi from '@/api/knowledge';
import type { DocumentItem, Category, LibraryType } from '@/api/knowledge';
import '../pages.css';
  ...
  useEffect(() => {
    if (initialDocId) {
      knowledgeApi.fetchDocDetail(initialDocId)
        .then((doc) => { setSelectedDoc(doc); setDrawerOpen(true); })
        .catch(() => {});
    }
  }, [initialDocId]);
    ...
    <PageContainer width="dashboard">
      <div data-module="knowledge-base">
        ...
        <nav className="kb-breadcrumb">
          ...
          <span className="kb-breadcrumb__current">文档查看</span>
        </nav>

        {/* 品牌标语（小） */}
        <div style={{ textAlign: 'center', padding: 'var(--spacing-sm) 0 var(--spacing-md)' }}>
          <Text level="l7" as="div" color="var(--color-neutral-500)">
            日耕朝夕，耕愈工作，耕暖生活
          </Text>
        </div>
        ...
        <div className="kb-list-layout">
          ...
          {/* 右侧：预览面板（抽屉式） */}
          <div className={`kb-preview-panel ${drawerOpen ? 'kb-preview-panel--open' : ''}`}>
            {drawerOpen && (
              <button className="kb-preview-close" onClick={closeDrawer}>✕</button>
            )}
            {selectedDoc ? (
              <>
