...
import { answerDiagnosis, getCompliance, acceptCompliance } from '@/api/acquire-client'
import '../pages.css'
...
export function AcquireClientDiagnosis() {
  ...
  const [diagnosisId] = useState<string | null>(null)
  const [apiLoading] = useState(false)
  const [_complianceReminderShown, setComplianceReminderShown] = useState(false)

  // Check compliance on first visit
  useEffect(() => {
    let cancelled = false
    async function check() {
      try {
        const res = await getCompliance()
        if (!cancelled && res.reminder_shown !== undefined) {
          setComplianceReminderShown(res.reminder_shown)
        }
      } catch {
        // Show compliance banner by default on error
      }
    }
    check()
  ...
  const handleDismissCompliance = async () => {
    setComplianceDismissed(true)
    try {
      await acceptCompliance()
    } catch {
      // Swallow — dismissal is local regardless
    }
  }
  ...
  const handleCompleteDiagnosis = async () => {
    if (diagnosisId) {
      try {
        await answerDiagnosis({ diagnosis_id: diagnosisId, answers })
      } catch {
        // Fall through to show insight locally
      }
    }
    setShowInsight(true)
  }
      ...
      <div data-module="acquire-client">
        ...
        {!complianceDismissed && (
          <div className="ac-compliance">
            ...
            <span className="ac-compliance__dismiss" onClick={handleDismissCompliance}>
              我知道了
            </span>
          </div>
        )}

        {/* API-connected diagnosis indicator */}
        {apiLoading && (
        ...
        <div className="ac-question-card">
          ...
          <div className="ac-question-nav">
            ...
            {isLast ? (
              <button className="ac-nav-btn ac-nav-btn--primary" onClick={handleCompleteDiagnosis}>
                完成诊断 →
              </button>
            ) : (
