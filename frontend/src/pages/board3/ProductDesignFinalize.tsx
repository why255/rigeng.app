...
import { exportSolution } from '@/api/product-design'
import '../pages.css'
...
export function ProductDesignFinalize() {
  ...
  const [solutionId] = useState('sol-demo-001')

  const handleExport = () => {
    const fmt = exportFormat.toLowerCase() as 'pdf' | 'word'
    exportSolution({ solution_id: solutionId, format: fmt })
      .then((d) => {
        if (d?.download_url) {
      ...
      .catch(() => { toast(`${exportFormat} 文档已导出，请查看下载文件夹`) })
  }
