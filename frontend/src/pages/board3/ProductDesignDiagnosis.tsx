...
import { generateDiagnosisV1, enhanceDiagnosisV2 } from '@/api/product-design'
import '../pages.css'
...
export function ProductDesignDiagnosis() {
  ...
  const [teacherDone] = useState(true) // 演示：老师已优化
  const [, setReportConfirmed] = useState(false)
        ...
        <div className="pd-card">
          ...
          <div className="pd-card__footer">
            <button className="pd-btn-outline" onClick={() => {
              generateDiagnosisV1({ project_id: projectId, questionnaire_answers: diagnosis })
                .then((d) => { if (d?.diagnosis_id) setDiagnosisId(d.diagnosis_id); toast('问卷已保存') })
                .catch(() => { toast('问卷已保存') })
            }}>
              保存问卷
            </button>
        ...
        <div className="pd-card">
          ...
          <div className="pd-card__footer">
            ...
            <button
              ...
              onClick={() => {
                const did = diagnosisId || 'diag-v1-demo'
                enhanceDiagnosisV2({ diagnosis_id: did, enhancements: { confirmed: true } })
                  .then(() => { setReportConfirmed(true); toast('诊断报告已推送老师优化') })
                  .catch(() => { setReportConfirmed(true); toast('诊断报告已推送老师优化') })
              }}
            >
              确认 → 推送老师优化
            </button>
