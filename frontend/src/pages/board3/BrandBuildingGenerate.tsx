...
import { generateArticle, confirmArticle } from '@/api/brand'
import type { ArticleContent } from '@/api/brand'
import '../pages.css'
...
/** 将 API 返回的 ArticleContent 映射到展示用的结果格式 */
function mapArticleToResult(a: ArticleContent) {
  return {
    title: a.title,
    wordCount: a.body?.length || 0,
    readTime: `${Math.max(1, Math.round((a.body?.length || 0) / 400))}分钟`,
    toneMatch: 92,
    tags: [],
    content: a.body || '',
  }
...
export function BrandBuildingGenerate() {
  ...
  const handleGenerate = () => {
    ...
    setGenerating(true)
    generateArticle({ topic: prompt.trim(), style: activeTab })
      .then((article) => {
        setContentId(article.content_id)
        setResult(mapArticleToResult(article))
        setShowResult(true)
  ...
  const handleConfirm = () => {
    if (!contentId) {
      toast('暂无可确认的内容', 'warning')
      return
    }
    confirmArticle({ content_id: contentId, action: 'confirm' })
      .then(() => toast('已确认发布', 'success'))
      .catch(() => toast('确认失败，请重试', 'error'))
  }
    ...
    <PageContainer width="dashboard">
      <div data-module="brand-building">
        ...
        <div className="bb-input-card">
          ...
          <button className="bb-generate-btn" onClick={handleGenerate} disabled={generating}>
            {generating ? '⏳ 生成中...' : '✨ 生成内容'}
          </button>
        </div>
        ...
        <div className="bb-action-row">
          <button className="bb-action-btn bb-action-btn--primary" onClick={handleConfirm}>
            🚀 立即发布
          </button>
