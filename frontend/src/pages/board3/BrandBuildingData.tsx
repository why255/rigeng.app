...
import { getBrandAnalytics, markLead } from '@/api/brand'
import type { BrandAnalytics } from '@/api/brand'
import '../pages.css'
...
export function BrandBuildingData() {
  ...
  const handleMarkLead = () => {
    markLead({ content_id: 'latest', lead_type: 'consultation' })
      .then(() => toast('已标记为潜在客户', 'success'))
      .catch(() => toast('标记失败', 'error'))
  }

  // 从 API 数据覆盖指标（有数据时优先使用 API）
  const displayMetrics = analytics
    ? [
        { key: 'published', label: '发布篇数', value: analytics.trends?.length || 12, trend: '+20%', trendUp: true },
        { key: 'reads', label: '阅读总数', value: analytics.views, trend: '+15%', trendUp: true },
        { key: 'likes', label: '点赞总数', value: analytics.likes, trend: '+8%', trendUp: true },
        { key: 'shares', label: '分享总数', value: analytics.shares, trend: '+12%', trendUp: true },
      ]
    : METRICS
