...
import { getBrandAnalytics, getCourageValue } from '@/api/brand'
import '../pages.css'
import './brand-building.css'

/** M8 品牌打造中心数据常量（作为初始状态，后续由 API 更新） */
const INIT_STATS = {
  courageValue: 128,
  courageMax: 500,
  nextTier: '持续发光者',
...
/** 从 CourageValue API 数据推导下一等级所需文章数 */
function deriveArticlesNeeded(milestones: any[]): number {
  if (!milestones || milestones.length === 0) return 2
  const next = milestones.find((m: any) => m.status === 'pending' || m.remaining)
  return next?.remaining ?? next?.articles_needed ?? 2
}
...
export function BrandBuildingEntry() {
  ...
  useEffect(() => {
    getCourageValue()
      .then((d) => {
        if (d) {
          setStats((prev) => ({
            ...prev,
            courageValue: d.total_score,
            nextTier: d.level || prev.nextTier,
            articlesNeeded: deriveArticlesNeeded(d.milestones),
          }))
        }
      })
      .catch(() => {})
    getBrandAnalytics()
      .then((d) => {
        if (d) {
          // 从趋势数据中推算本周发布数（取最近趋势项的计数）
          const fallbackWeekly = INIT_STATS.weeklyPublished
          const recentCount = d.trends?.length || fallbackWeekly
          setStats((prev) => ({
            ...prev,
            weeklyPublished: recentCount > 0 ? Math.min(recentCount, prev.weeklyTarget) : prev.weeklyPublished,
          }))
        }
      })
      .catch(() => {})
