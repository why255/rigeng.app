...
/** 获取最近文档列表 */
export function fetchRecentDocs(limit: number = 5, _sort: string = 'latest'): Promise<DocumentItem[]> {
...
/** 丢弃文档（从审核队列永久移除） */
export function discardDoc(id: string): Promise<{ success: boolean }> {
  return apiPost<{ doc_id: string }>(`/kb/documents/${id}:discard`)
    .then(res => ({ success: !!res.doc_id }));
}
...
function mapKpi(raw: KpiRaw): KpiData {
  return {
    ...
    completionRateChange: 0, // 由前端从趋势数据对比计算
    sopCount: raw.sop_count,
    sopCountChange: 0,
    streakDays: raw.streak_days,
    // 附加字段
    emotionScore: raw.emotion_score,
    totalRecordings: raw.total_recordings,
    totalDocs: raw.total_docs,
    courageValue: raw.courage_value,
